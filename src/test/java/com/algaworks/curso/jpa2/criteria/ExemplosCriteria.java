...
public class ExemplosCriteria {
	...
	public void exemploFuncao() {
		...
		Root<Carro> carro = criteriaQuery.from(Carro.class);
		Predicate predicateCor = builder.equal(builder.upper(carro.<String>get("cor")), "preto".toUpperCase());
		Predicate predicatePlaca = builder.equal(builder.upper(carro.<String>get("placa")), "aaa1321".toUpperCase());
		
		criteriaQuery.select(carro);
		criteriaQuery.where(predicateCor, predicatePlaca);
	...
	public void exemploJoinEFetch() {
		...
		criteriaQuery.select(carro);
		criteriaQuery.where(builder.equal(modelo.get("descricao"), "Fusca"));
	...
	@Test
	/**
	 * O Metamodel é um plugin do maven que cria classes metamodelos para evitar o uso das propriedades no criteria utilizando strings. 
	 */
